...
bool TW_Protocol::ConstructFrame(eSubCmdType_t subCmdType, const string& jsonData, u8* pcucDstData, u32& nDstLen)
{
	...
	// 帧头
	pcucDstData[0] = eProtocolConst::PROTOCOL_HEADER;
	// 数据长度
	u32 DataLen = jsonData.size();
	pcucDstData[1] = 0xFF & (DataLen >> 8);
	pcucDstData[2] = 0xFF & DataLen;
	// 本帧编号
	pcucDstData[3] = eProtocolConst::PROTOCOL_NUMBER;
	pcucDstData[4] = eProtocolConst::PROTOCOL_NUMBER;
	// 命令大类
	...
	//帧尾
	pcucDstData[nDstLen - 1] = eProtocolConst::PROTOCOL_END;
...
bool TW_Protocol::ParseProtocolData(const u8 * pcucSrcData, int nSrcLen, eSubCmdType_t& correctSubCmd, string & jsonData)
{
	...
	// 开始解析data
	if (eProtocolConst::PROTOCOL_HEADER != pcucSrcData[0] ||
		eProtocolConst::PROTOCOL_NUMBER != pcucSrcData[3] ||
		eProtocolConst::PROTOCOL_NUMBER != pcucSrcData[4])
	{
		return false;
	}
	...
	//	return false;
	//}
	correctSubCmd = (eSubCmdType_t)pcucSrcData[6];


	unsigned int nDstLen = (0xFF00 & (pcucSrcData[1] << 8)) + (0xFF & pcucSrcData[2]);
	if (eProtocolConst::PROTOCOL_END != pcucSrcData[nDstLen + 10 - 1])
	{
		return false;
	}
